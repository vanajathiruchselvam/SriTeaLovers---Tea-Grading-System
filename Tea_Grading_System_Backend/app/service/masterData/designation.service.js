...
const log = log4js.getLogger('designation.service.js');
...
exports.getAll = (page, itemPerPage, query, status) => new Promise(async(resolve, reject) => {
    ...
    let responce = await BaseService.paginate(Designation, page, itemPerPage, log,  condition, attributes );
    resolve(responce)
...
exports.create = (values) =>new Promise(async(resolve, reject) => {
  let responce = await BaseService.create(Designation, values, log);
  resolve(responce)
});
...
exports.show = (model) => new Promise(async(resolve, reject) => {
  let attributes = ['id', 'name', 'is_active', 'created_at'];
  let responce = await BaseService.show(Designation, model, log, attributes);
  resolve(responce)
});
...
exports.update = (id, values) => new Promise(async(resolve, reject) => {
    ...
    let responce = await BaseService.update(Designation, id, values, log);
    resolve(responce)
});
...
exports.delete = (id) => new Promise(async(resolve, reject) => {
    if (!id) reject(new Error(`id can't be empty`));
    let responce = await BaseService.delete(Designation, id, log);
    resolve(responce)
});
...
exports.search = (query, except, status) => new Promise( async(resolve, reject) => {
    ...
    let values = await BaseService.search(Designation, log, condition, attributes);
    resolve(values)
});
